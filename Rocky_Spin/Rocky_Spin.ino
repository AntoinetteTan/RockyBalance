...
const uint16_t ADVANCED_INTERVAL = 50000;  // Time interval between doing advance movement
uint32_t advanced_time;                  // Global variable to store the time that rocky has last done advanced movement
...
void BalanceRocky()
{
  ...
  // *** enter equations for input signals for v_c (left and right) in terms of the variables available ****
    v_c_R = v_d - Jp*measured_speedR-(JiCp)*distLeft_m-Ci*dist_accum;
    v_c_L = v_d - Jp*measured_speedL-(JiCp)*distRight_m-Ci*dist_accum;  
